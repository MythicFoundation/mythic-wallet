...
export interface StoredWallet {
  encryptedMnemonic: string;  // encrypted mnemonic OR private key
  publicKey: string;
  createdAt: number;
  importType?: 'mnemonic' | 'privatekey';  // how wallet was imported
  mythDomain?: string;  // e.g. "alice.myth"
}
...
export async function saveWallet(
  ...
  const encryptedMnemonic = await encryptData(secret, password);
...
export async function saveMythDomain(domain: string): Promise<void> {
  const wallet = await getWallet();
  if (!wallet) return;
  await chrome.storage.local.set({
    [STORAGE_KEYS.WALLET]: { ...wallet, mythDomain: domain },
  });
}
