...
export default function TransactionHistory({ address, network, onBack }: TransactionHistoryProps) {
        ...
        for (let i = 0; i < parsedTxs.length; i++) {
          ...
          for (const ix of instructions) {
            if ('parsed' in ix && ix.program === 'system' && ix.parsed?.type === 'transfer') {
              ...
              if (source === address) {
                txRecords[i] = { ...txRecords[i], type: 'send', amount, to: truncateAddress(dest, 4) };
              } else if (dest === address) {
                txRecords[i] = { ...txRecords[i], type: 'receive', amount, from: truncateAddress(source, 4) };
              }
              break;
            }
          }
