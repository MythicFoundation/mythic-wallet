...
export default function Settings({
  ...
  const handleExport = async () => {
    ...
    try {
      const { verifyPassword, getWallet, decryptData } = await import(
        "../../lib/storage"
      );
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-4 border-b border-subtle">
          <h3 className="text-xs text-text-muted mb-3 font-sans uppercase tracking-wider">
            Network
          </h3>
          <div className="space-y-2">
            {(
              Object.entries(NETWORKS) as [
                NetworkId,
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      network === id ? "bg-rose" : "bg-text-disabled"
                    }`}
                  />
                  <span className="text-sm font-display text-text-heading">
                    {net.name}
                  </span>
                </div>
                {network === id && (
          ...
          {connectedSites.length === 0 ? (
            ...
            <div className="space-y-2">
              {connectedSites.map((site) => (
                <div
                  key={site}
                  className="flex items-center justify-between px-3 py-2 bg-surface-elevated border border-subtle"
                >
        ...
        <div className="px-4 py-4 border-b border-subtle">
          ...
          >
            <span className="text-sm text-text-heading font-display">
              Export Private Key
            </span>
            <svg
          ...
          {showExport && (
            <div className="px-3 py-3 bg-surface-elevated border border-subtle">
              <p className="text-[10px] text-error mb-2">
                Warning: Never share your private key. Anyone with it can steal
                your funds.
              </p>
              {exportedKey ? (
                <div className="mb-2">
                  <div className="bg-surface-card border border-subtle px-3 py-2 break-all">
                    <p className="font-mono text-[10px] text-text-heading select-all">
                      {exportedKey}
                    </p>
                  </div>
                  <button
                ...
                <>
                  ...
                  />
                  {exportError && (
                    <p className="text-[10px] text-error mb-2">{exportError}</p>
                  )}
                  <Button
                    variant="secondary"
                    size="sm"
